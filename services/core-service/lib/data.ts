...
import { getApplicationSummary } from "./application-service";
...
export async function getDashboardData() {
  ...
  const applicationSummaryPromise = getApplicationSummary(undefined, 5);
  ...
  const pendingApplications = applicationSummary.pendingCount;
  const recentApplications = applicationSummary.recentApplications.map((application) => ({
    ...application,
    form: {

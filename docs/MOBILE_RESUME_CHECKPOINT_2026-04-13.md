...
2. ✅ 에이전트 성장 로그를 GitHub 이슈/작업 큐로 실제 발행하는 브리지 추가
   - 반영 커밋: `f4c4c2e`
   - 내용: `/api/mobile/agent-growth`에 POST 발행 API 추가, GitHub 토큰/레포 설정 시 실제 이슈 생성, 미설정 시 작업 큐(ActivityLog) 저장으로 폴백. 모바일 월드 탭에 `이슈/작업큐 발행` 버튼과 발행 상태 표시 추가.
3. ✅ 월드 오브젝트 스프라이트를 실제 이미지 에셋(2D 픽셀)로 치환
   - 반영 커밋: `5342540`
   - 내용: `PixelSprite`를 임시 픽셀 패턴 렌더링에서 이미지 스프라이트 렌더링으로 전환하고, `apps/mobile-app/assets/sprites/{hub,house,person}` 에셋 추가

...
import {
  BetaShepherd,
  addShepherdToStorage,
  parseShepherdCommand,
  readShepherdsFromStorage,
} from "@/lib/beta-shepherding-client";

export default function BetaWorldPage() {
  const [command, setCommand] = useState("목자 추가 김은혜, 새가족 2가정 확인");
  const [message, setMessage] = useState("월드 안에서 바로 명령해봐. 목양 공간이 같이 반응하게 붙였어.");
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const parsed = parseShepherdCommand(command);

      if (!parsed) {
        setMessage("지금은 `목자 추가 이름, 메모` 형식으로 먼저 반응하게 해놨어.");
        return;
      }

      const shepherd = addShepherdToStorage(parsed);
      refreshShepherds();
      setMessage(`${shepherd.name} 목자 카드가 목양 공간에 추가됐어.`);
      setCommand("");
    } catch (error) {
      ...
      <div className="absolute right-4 top-[108px] z-20 hidden w-[340px] lg:block xl:right-6">
        <div className="rounded-[24px] border border-white/14 bg-black/12 p-4 text-white backdrop-blur-md shadow-[0_18px_40px_rgba(0,0,0,0.14)]">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[10px] tracking-[0.18em] text-white/46">SHEPHERDING PREVIEW</p>
          ...
          <div className="mt-4 grid gap-3">
            {shepherds.slice(0, 3).map((shepherd) => (
              <div key={shepherd.id} className="rounded-[18px] border border-white/10 bg-white/[0.06] p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-white">{shepherd.name}</p>
                      <p className="text-[12px] text-white/58">{shepherd.note}</p>
                    </div>
                  </div>
                  <span className="rounded-full border border-white/10 bg-black/10 px-2 py-1 text-[10px] text-white/64">{shepherd.status}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          >
            ...
            <div className="mt-4 flex flex-col gap-3">
              <textarea
                ...
                placeholder="목자 추가 김은혜, 새가족 2가정 확인"
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex flex-wrap gap-2 text-[12px] text-white/62">
                  <span className="rounded-full border border-white/10 bg-white/[0.05] px-3 py-1.5">예: 목자 추가 김은혜, 새가족 2가정 확인</span>
                  <span className="rounded-full border border-white/10 bg-white/[0.05] px-3 py-1.5">예: 목자 추가 박요한, 청년 목장 방문 가능</span>
                </div>
                <button
                  type="submit"
          ...
          <div className="rounded-[26px] border border-white/14 bg-black/14 p-4 text-white backdrop-blur-md shadow-[0_18px_40px_rgba(0,0,0,0.16)] sm:p-5">
            ...
            <div className="mt-4 rounded-[18px] border border-white/10 bg-white/[0.05] p-3 text-[12px] leading-5 text-white/58">
              지금은 현재 브라우저 기준으로 목양 카드가 바로 갱신되게 붙여놨어. 다음엔 공간 열기, 목자 선택, 목원 입장까지 이어가면 돼.
            </div>
          </div>
        </div>
      </div>

...
export default async function AppEntryPage() {
  ...
  const demoChurch = await getChurchBySlug("demo-soom");

  return (
    <main className="mx-auto w-full max-w-2xl px-4 py-10 sm:px-6">
      <div className="rounded-xl border border-border bg-card p-5">
        <h1 className="text-2xl font-bold">아직 연결된 교회 워크스페이스가 없습니다</h1>
        <p className="mt-2 text-sm text-muted-foreground">새 교회를 만들거나 초대받은 교회에 참여하세요.</p>
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-sm">
          {demoChurch ? (
            <Link href={`/app/${demoChurch.slug}/dashboard`} className="rounded-md bg-primary px-3 py-2 text-white">데모 워크스페이스 보기</Link>
          ) : null}

...
function getPrimaryLinks(churchSlug: string, memberId: string, statusTag: string) {
  const base = `/app/${churchSlug}/members/${memberId}`;
  const common = [
    { href: `${base}/edit`, label: "기본 정보 수정" },
    { href: base, label: "상세 기록 열기" },
  ];

  if (statusTag === "등록대기") {
    return [{ href: `${base}/edit`, label: "배정 정보 입력" }, ...common];
  }
  if (statusTag === "새가족") {
    return [{ href: base, label: "첫 연락 기록" }, ...common];
  }
  if (statusTag === "정착중") {
    return [{ href: base, label: "정착 상태 기록" }, ...common];
  }
  if (statusTag === "목장배정완료") {
    return [{ href: `${base}/edit`, label: "목장 정보 확인" }, ...common];
  }
  if (statusTag === "봉사연결") {
    return [{ href: base, label: "사역 연결 확인" }, ...common];
  }
  if (statusTag === "심방필요") {
    return [{ href: base, label: "심방 기록 열기" }, ...common];
  }
    ...
    <div className="flex flex-col gap-6 text-[#111111]">
      <section className="grid gap-4 xl:grid-cols-[1.12fr_0.88fr]">
        <div className="rounded-[28px] border border-[#e1d7c7] bg-[linear-gradient(135deg,#10192d_0%,#17233d_55%,#243252_100%)] p-6 text-white shadow-[0_18px_50px_rgba(15,23,42,0.16)] sm:p-7">
          ...
          <p className="mt-3 max-w-xl text-sm leading-6 text-white/70">등록 직후 바로 정리해야 할 상태와 배정 항목을 먼저 확인하는 화면입니다.</p>
          <div className="mt-5 rounded-[18px] border border-white/10 bg-white/8 p-4">
            <p className="text-[11px] tracking-[0.16em] text-white/46">현재 단계</p>
        ...
        <section className="rounded-[28px] border border-[#e5dccd] bg-[#fbfaf6] p-5 shadow-[0_12px_32px_rgba(15,23,42,0.06)] sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-xl font-semibold text-[#111111]">다음 작업</h2>
            </div>
            <span className="rounded-full border border-[#eadfcd] bg-white px-3 py-1 text-[11px] text-[#8C7A5B]">action</span>
          </div>
          <div className="mt-4 grid gap-3">
            {primaryLinks.map((item) => (
              <Link key={item.label} href={item.href} className="rounded-[18px] border border-[#ece6dc] bg-white p-4 text-sm font-semibold text-[#111111] transition hover:bg-[#fcfbf8]">{item.label}</Link>
            ))}
      ...
      <section className="grid gap-4 xl:grid-cols-[0.92fr_1.08fr]">
        <section className="rounded-[24px] border border-[#e6dfd5] bg-white p-5 shadow-[0_10px_28px_rgba(15,23,42,0.05)]">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="text-xs text-[#8C7A5B]">운영 단계 반영</span>
          </div>
          ...
          <div className="mt-4 grid gap-3">
            <div className="rounded-[16px] border border-[#ece6dc] bg-[#fcfbf8] p-4 text-sm text-[#111111]">다음 작업 · {statusMeta.nextAction}</div>

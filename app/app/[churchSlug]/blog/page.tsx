            ...
            posts.map((post) => (
              <div key={post.id} className="rounded-[18px] border border-[#ede6d8] bg-[#fcfbf8] p-4">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <Link href={`/app/${church.slug}/blog/${post.id}/edit`} className="rounded-[12px] border border-[#E7E0D4] bg-white px-3 py-2 text-xs font-medium text-[#121212]">기록 수정</Link>
                  <Link href={`/app/${church.slug}/blog/${post.id}/preview`} className="rounded-[12px] border border-[#E7E0D4] bg-white px-3 py-2 text-xs font-medium text-[#121212]">내용 보기</Link>
                </div>
              </div>
            ))
          )}

...
export function WorkspaceMemberForm({ member, action, districts, groups, households }: WorkspaceMemberFormProps) {
  return (
    <form action={action} className="grid grid-cols-1 gap-4 rounded-[24px] border border-[#e6dfd5] bg-white p-5 shadow-[0_10px_28px_rgba(15,23,42,0.05)] lg:grid-cols-2">
      ...
      <label className="text-sm font-medium text-[#3f3528]">가족<select name="householdId" defaultValue={member?.householdId ?? ""} className="mt-1 w-full rounded-[14px] border border-[#E7E0D4] bg-[#fcfbf8] px-3 py-2.5 text-sm text-[#111111] outline-none transition focus:border-[#d6c3a1] focus:bg-white"><option value="">선택 안함</option>{households.map((h) => <option key={h.id} value={h.id}>{h.name}</option>)}</select></label>
      ...
      <label className="text-sm font-medium text-[#3f3528]">목장<select name="groupId" defaultValue={member?.groupId ?? ""} className="mt-1 w-full rounded-[14px] border border-[#E7E0D4] bg-[#fcfbf8] px-3 py-2.5 text-sm text-[#111111] outline-none transition focus:border-[#d6c3a1] focus:bg-white"><option value="">선택 안함</option>{groups.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}</select></label>

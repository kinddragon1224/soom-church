    ...
    <section className="border-t border-white/10">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 px-4 py-14 sm:px-6 sm:py-16 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex gap-3">
          <Link href={action.primaryHref} className="rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-black">{action.primaryLabel}</Link>
          <Link href={action.secondaryHref} className="rounded-full border border-white/30 px-5 py-2.5 text-sm font-medium text-white">{action.secondaryLabel}</Link>
        </div>
      </div>
    </section>
  );
